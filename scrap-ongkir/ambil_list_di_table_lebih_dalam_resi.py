...
def scrape_page(url, writer):
    ...
    for question in questions:
        ...
        if question_page_soup:
            ...
            writer.writerow({'Title': title, 'URL': url, 'Message': message, 'Category': category, 'Tracking Number': tracking_number})
...
# Fungsi untuk mengekstrak nomor resi dari pesan
def extract_tracking_number(message):
    patterns = [
        r'(no\.?\s*resi|nomor\s*resi)\s*([A-Za-z0-9]+)',
        r'(No\s*resi|resi)\s*:\s*([A-Za-z0-9]+)',
        r'(dilacak|Resi)\s*([A-Za-z0-9]+)',
        r'(JP|IDD|TKP01|JB)\d+',
        r'\d{13,}'
    ]
...
start_page = 20
max_page = 40  # Jumlah maksimum halaman yang akan di-scrape

# Membuka file CSV untuk menyimpan data
with open('data_tanyapaket_test.csv', 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['Title', 'URL', 'Message', 'Category', 'Tracking Number']
